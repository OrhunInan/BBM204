...
public class Project implements Serializable {
    ...
    public static boolean containsFalse(boolean[] nodes){

        for(boolean node : nodes) if(!node) return true;
        return false;
    }
    ...
    public int[] getEarliestSchedule() {
        ...
        Queue<Task> next = new LinkedList<>();

        next.add(current);

        while (!next.isEmpty()){
            int currentTaskID = current.getTaskID();
            int currentTaskDuration = current.getDuration();

            tasks.stream().filter(task -> task.getDependencies().contains(currentTaskID)).forEach(task -> {
                next.add(task);

                startTimes[task.getTaskID()] =
                        Math.max(startTimes[task.getTaskID()], startTimes[currentTaskID] + currentTaskDuration);
            });
